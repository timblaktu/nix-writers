...
let
  ...
  # Define bash scripts using the mkBashScript helper
  bashScripts = {
    
    # Example script to demonstrate the infrastructure
    # This will be replaced with real scripts in Phase 2
    ...
    check-terminal-setup = mkBashScript {
      ...
      text = /* bash */ ''
        ...
        echo "Settings: ''${WT_SETTINGS_PATH:-/mnt/c/Users/*/AppData/Local/Packages/Microsoft.WindowsTerminal_8wekyb3d8bbwe/LocalState/settings.json}"
        echo
        ...
        if [[ -n "$WSL_DISTRO_NAME" ]] || [[ -n "$WSLENV" ]]; then
          if [[ -f "''${WT_SETTINGS_PATH}" ]]; then
            ...
            echo "  Expected Font: ''${WT_EXPECTED_FONT:-Cascadia Code, Noto Color Emoji, Segoe UI Emoji}"
    ...
    setup-terminal-fonts = mkBashScript {
      ...
      text = /* bash */ ''
        ...
        if [[ -f "''${WT_SETTINGS_PATH}" ]]; then
          ...
          if [[ "$CURRENT_FONT" == "''${WT_EXPECTED_FONT:-"Cascadia Code, Noto Color Emoji, Segoe UI Emoji"}" && "$INTENSE_STYLE" == "all" ]]; then
            ...
            echo '   "profiles": {'
            echo '     "defaults": {'
            echo "       \"font\": { \"face\": \"''${WT_EXPECTED_FONT:-"Cascadia Code, Noto Color Emoji, Segoe UI Emoji"}\" },"
          ...
          echo "   Expected location: ''${WT_SETTINGS_PATH:-/mnt/c/Users/*/AppData/Local/Packages/Microsoft.WindowsTerminal_*/LocalState/settings.json}"
    ...
    tmux-session-picker = mkBashScript {
      ...
      deps = with pkgs; [ tmux fzf coreutils findutils gnugrep gawk gnused ];
      text = /* bash */ ''
        #!/usr/bin/env bash
        
        # tmux-session-picker: Interactive tmux resurrect session picker using fzf
        # Usage: tmux-session-picker [--preview-only]
        ...
        # Function to extract session info from resurrect file
        extract_session_info() {
            local file="$1"
            local basename=$(basename "$file" .txt)
            local timestamp=$(echo "$basename" | sed 's/tmux_resurrect_//')
            
            # Parse timestamp (format: 20250815T204251)
            ...
            local windows=$(grep "^window" "$file" | wc -l)
            local panes=$(grep "^pane" "$file" | wc -l)
            
            # Get session name(s)
            local sessions=$(grep "^window" "$file" | cut -f2 | sort -u | tr '\n' ',' | sed 's/,$//')
            ...
            printf "%-20s │ %-19s │ %2d windows │ %2d panes │ %s%s\n" \
                "$sessions" "$formatted_date" "$windows" "$panes" "$basename" "$current"
        }
        
        # Function to preview session content
        preview_session() {
            local file="$1"
            echo "=== Session Preview: $(basename "$file") ==="
            echo
            ...
            # Show windows
            echo "Windows:"
            grep "^window" "$file" | while IFS=$'\t' read -r type session window_id name active flags layout; do
                local status=""
                [[ "$active" == "1" ]] && status="*" || status=" "
                echo "  [$status] Window $window_id: $name"
            done
            echo
            
            # Show panes with their working directories
            echo "Panes and Working Directories:"
            grep "^pane" "$file" | while IFS=$'\t' read -r type session window_id pane_id name active title path focused cmd rest; do
                local status=""
                [[ "$focused" == "1" ]] && status="*" || status=" "
                echo "  [$status] $session:$window_id.$pane_id - $path ($cmd)"
            done
        }
        
        # Function to get session files sorted by date (newest first)
        get_session_files() {
            find "$RESURRECT_DIR" -name "tmux_resurrect_*.txt" -type f | sort -r
        }
        
        # Main logic
        main() {
            local session_files
            readarray -t session_files < <(get_session_files)
            
            if [[ ''${#session_files[@]} -eq 0 ]]; then
                echo "No tmux resurrect sessions found in $RESURRECT_DIR"
                exit 1
            fi
            
            # If preview-only mode, just show all sessions
            if [[ "$PREVIEW_ONLY" == "true" ]]; then
                echo "Available tmux resurrect sessions:"
                echo
                printf "%-20s │ %-19s │ %-10s │ %-9s │ %s\n" \
                    "SESSION" "DATE" "WINDOWS" "PANES" "FILE"
                printf "%-20s─┼─%-19s─┼─%-10s─┼─%-9s─┼─%s\n" \
                    "────────────────────" "───────────────────" "──────────" "─────────" "──────────────────────────────"
            ...
            if ! command -v fzf >/dev/null 2>&1; then
                echo "Error: fzf is required for interactive session selection"
                echo "Install fzf or use --preview-only to list sessions"
                exit 1
            fi
            ...
            local fzf_input=""
            for file in "''${session_files[@]}"; do
                fzf_input+="$(extract_session_info "$file")\t$file\n"
            done
            ...
            selected=$(echo -e "$fzf_input" | fzf \
                ...
                --layout=reverse \
                --border \
                --header="Select tmux session to restore (Current session marked with [CURRENT])" \
                --preview="$(printf "%q" "$0") --preview-file {-1}" \
            ...
            local selected_file=$(echo "$selected" | cut -f2)
            ...
            echo "Setting '$selected_file' as the session to restore..."
        ...
        # Special case for preview file argument (used by fzf preview)
        if [[ "''${1:-}" == "--preview-file" && -n "''${2:-}" ]]; then
            preview_session "$2"
            exit 0
        fi
      ...
      tests = {
        syntax = writers.testBash "test-tmux-session-picker-syntax" ''
          echo "✅ Syntax validation passed at build time"
        '';
        argument_parsing = writers.testBash "test-tmux-session-picker-args" ''
          # Test argument parsing logic - placeholder
          echo "✅ Argument parsing test passed (placeholder)"
        '';
        fzf_integration = writers.testBash "test-tmux-session-picker-fzf" ''
          # Test fzf integration and preview functionality - placeholder
          echo "✅ FZF integration test passed (placeholder)"
        '';
        resurrect_parsing = writers.testBash "test-tmux-session-picker-parsing" ''
          # Test tmux resurrect file parsing - placeholder
          echo "✅ Resurrect file parsing test passed (placeholder)"
        '';
      };
    };
    ...
    diagnose-emoji-rendering = mkBashScript {
      ...
      text = /* bash */ ''
        ...
        echo "  Font: ''${WT_EXPECTED_FONT:-CaskaydiaMonoNerdFontMono, Noto Color Emoji, Segoe UI Emoji}"
...
in {
  config = mkIf (cfg.enable && cfg.enableBashScripts) {
    validatedScripts.bashScripts = bashScripts;
  };
}